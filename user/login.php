<?php
    ...
    function Checkname($name_input){
        global $name, $id;
        $sql="SELECT * from `yy_user` where `uname`='$name_input'";
        $result=MySql_query($sql);
        $row=mysql_fetch_array($result);
        $name=$row["uname"];
        $id=$row["uid"];
    ...
    {
        $sql="select `uemail` from `yy_user` where `uid`='$id'";
        $result=MySql_query($sql);
        $row=mysql_fetch_array($result);
        return ($row["uemail"]);
    }
    function getIcon($id)
    {
        $sql="select `uimg` from `yy_user` where `uid`='$id'";
        $result=MySql_query($sql);
        $row=mysql_fetch_array($result);
        return ($row["uimg"]);
    }
    ...
    //if(!isset($_SESSION['id'])){
        if(true/*!empty($_POST['submit'])*/)
        {
            ...
            if(!isset($error))
            {
                ...
                //mysql_query($sql);
                $sql="UPDATE `yy_user` SET `ustatus` = '1' WHERE `uid` = '$id'";
                mysql_query($sql);
